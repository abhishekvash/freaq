...
- If custom CSS is unavoidable for a one-off view/component, put it in a scoped `<style scoped>` block inside that Vue file. Shared CSS belongs in `app/assets/css/tailwind.css` only when it is truly a reusable token, theme value, or base rule.
