...
const staticWave = (() => {
  ...
  for (let i = 0; i < n; i++) {
    const x = i / (n - 1)
    const breath =
      0.55 + 0.45 * Math.sin(x * Math.PI * 3.7) * Math.cos(x * Math.PI * 1.3)
  ...
  function frame() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const bump =
        pulseStrength > 0.01 && dist < 260
          ? 1 + 0.18 * pulseStrength * Math.exp(-(dist * dist) / twoSigmaSq)
          : 1
      xs[i] = x
      vs[i] = points[i] * breath * bump
    }

    ctx.strokeStyle = 'rgba(232,226,216,0.16)'
    ctx.lineWidth = 1.1
    ctx.beginPath()
    for (let i = 0; i < n; i++) {
      const y = midY - amp * vs[i] * 0.5
    ...
    if (pulseStrength > 0.01) {
      ...
      for (let i = 0; i < n - 1; i++) {
        ...
        ctx.beginPath()
        ctx.moveTo(xs[i], midY - amp * vs[i] * 0.5)
        ctx.lineTo(xs[i + 1], midY - amp * vs[i + 1] * 0.5)
        ctx.stroke()
        ctx.beginPath()
        ctx.moveTo(xs[i], midY + amp * vs[i] * 0.5)
        ctx.lineTo(xs[i + 1], midY + amp * vs[i + 1] * 0.5)
...
onMounted(async () => {
  ...
  const { animate } = motionMod as any
  ...
  segments.forEach((seg) => {
    ...
    tl.to(
      seg.ref.value,
      { duration: dur, text: { value: seg.text, delimiter: '' }, ease: 'none' },
      cursor
    )
    if (bracketRef.value) {
      tl.to(bracketRef.value, { duration: 0.06, opacity: 0.35 }, cursor)
      tl.to(bracketRef.value, { duration: 0.18, opacity: 1 }, cursor + 0.06)
    }
    cursor += dur + 0.05
  })
  ...
  tl.add(() => {
    if (line1Ref.value)
      animate(line1Ref.value, { opacity: 1, y: 0 }, spring)
  }, 0.72)
  tl.add(() => {
    if (line2Ref.value)
      animate(line2Ref.value, { opacity: 1, y: 0 }, spring)
  }, 0.88)
  tl.add(() => {
    if (line3Ref.value)
      animate(line3Ref.value, { opacity: 1, y: 0 }, spring)
  }, 1.04)
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <span ref="bracketRef" class="vault-bracket text-accent-dark">&gt;</span>
        <span ref="seg1Ref">{{ TEXT.seg1 }}</span><span ref="seg2Ref">{{
          TEXT.seg2
        }}</span><span ref="seg3Ref">{{ TEXT.seg3 }}</span>
      </p>
      ...
      >
        ...
        >
          ...
          <span ref="scanlineRef" class="vault-scanline" aria-hidden="true" />
        </span>
      </h1>

      <div ref="bodyRef" class="vault-body col-span-12 mt-section lg:col-span-7">
        <p class="max-w-[58ch] text-body text-muted-dark">
          Freaq is for music producers who want a deadline that bites. Every
          Friday the vault opens: one sealed seed, sixty minutes, one bounce.
