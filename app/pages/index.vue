...
const setPactItemRef = (index: number) => (el: any) => {
    if (el) pactItemRefs.value[index] = el as HTMLElement
}
...
onMounted(async () => {
    ...
    const motionMod = (await import('motion')) as any
    const { animate, inView } = motionMod
    ...
    pactItemRefs.value.forEach((el, i) => reveal(el, { delay: i * 0.06, y: 24 }))
...
<template>
    <div class="isolate min-h-screen bg-bg-dark text-text-dark antialiased">
        <header
            class="sticky top-0 z-10 border-b border-line-dark bg-bg-dark"
        >
            <div
                class="mx-auto flex max-w-7xl items-center justify-between gap-panel px-panel py-field"
            >
                <FreaqBrand href="#top" />
                <FreaqButton href="#join" variant="secondary">ENTER THE CIRCLE</FreaqButton>
            </div>
        </header>
        ...
        <section id="pact">
            <div class="mx-auto max-w-7xl px-panel pt-hero pb-hero">
                ...
                >
                    THE IDEA. <span class="text-accent-dark">CONSTRAINT AS INSTRUMENT.</span>
                </h2>
                ...
                <ol class="mt-section divide-y divide-line-dark">
                    ...
                    >
                        <span
                            class="pact-numeral col-span-12 font-mono text-stagger-2 leading-[0.85] font-bold tabular-nums text-muted-dark sm:col-span-3"
                        >
                            {{ item.n }}
                        </span>
        ...
        <section class="border-t border-line-dark bg-raised-dark shadow-vessel-edge">
            <div class="mx-auto max-w-7xl px-panel py-hero">
                <p
                    class="font-display font-bold tracking-normal uppercase"
                >
                    <span
                        ref="mantraLine1Ref"
                ...
                >
                    ...
                    and a reason to finish the idea while it still has
                    voltage.
                </p>
            </div>
        </section>
        ...
        <section id="join" class="border-t border-line-dark">
            ...
            >
                ...
                >
                    <div v-if="!submitted" class="max-w-[52ch] space-y-field text-body text-muted-dark">
                        <p>
                            I’m building Freaq solo. The platform isn’t live
                            yet. The circle is the list of producers who’ll
                            hear it first.
                        </p>
                        <p>
                            Drop your email. One signal lands the day the
                            vault opens.
                        </p>
                    </div>
        ...
        <footer ref="footerRef" class="border-t border-line-dark">
            <div
                class="mx-auto max-w-7xl px-panel py-section"
            >
                <FreaqBrand
                    mark-class="size-2"
                    text-class="text-lg"
...
.signal-stamp__sheet {
    ...
    border-radius: 4px;
    background: color-mix(in oklab, var(--stamp-color) 8%, var(--freaq-surface));
